		...
		<div class='panier'>
			<table><center>
				...
				for ($i=0; $i<=$NbrLigne; $i++) { 
					echo"<tr bgcolor=#aec5eb>";  // Première boucle for qui sert a mettre le bon nombre de ligne pour les produits -->
					for($j=0;$j<=$NbrCol;$j++) { 							   // Première boucle for qui sert a mettre le bon nombre de colonne pour les produits -->
						echo'<td><center>'; 
						echo ($tableau[$i][$j]); 
						echo"</center></td>";			   //Affiche toutes les infos du produit 
				 }
				echo "<td><center>"; $Prixtoto+=$tableau[$i][2]*$tableau[$i][3]; echo ($tableau[$i][2]*$tableau[$i][3]) ; 
				echo"</center></td>
				<td><center><form form='post'> <input name='page' value='panier' hidden> <input  type='submit' name='" .$i."'value='X' class='boutonrond'/></form><center></td>  		<!-- Créer un bouton rond qui contient la valeur de la ligne ce qui nous permet ensuite de la récupérer pour supprimer l'objet -->
				</tr>"; } //Calcul le prix total
				echo"</tbody>
				<tbody><tr bgcolor=#edafb8>
				<td colspan='7'><center>Prix Total :"; echo $Prixtoto; echo"</center></td>
			 	</tr></tbody>
				<tfoot><tr bgcolor=#edafb8>"; //Affiche le prix total de tout les produits
